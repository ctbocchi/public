...
void MissionManagerImplementation::handleMissionListRequest(MissionTerminal* missionTerminal, CreatureObject* player, int counter) {
	...
	if (missionTerminal->isBountyTerminal()) {
		if (!player->hasSkill("combat_bountyhunter_novice") && !player->hasSkill("mandalorian_master")) {
...
void MissionManagerImplementation::randomizeGenericBountyMission(CreatureObject* player, MissionObject* mission, const uint32 faction, Vector<ManagedReference<PlayerBounty*>>* potentialTargets) {
	...
	bool playerIsJedi = false;
	PlayerObject* playerGhost = player->getPlayerObject();

	if (playerGhost != nullptr)
		playerIsJedi = playerGhost->isJedi();

	if (player->hasSkill("combat_bountyhunter_novice") || (player->hasSkill("mandalorian_master") && !playerIsJedi)) {
		// do nothing
	}else{
        player->sendSystemMessage("@mission/mission_generic:not_bounty_hunter_terminal");
        return;
	...
	int level = 1;
	int randomTexts = 25;
	if (player->hasSkill("combat_bountyhunter_investigation_03") || player->hasSkill("mandalorian_master")) {
		level = 3;
	...
	if (playerTarget) {
		...
		if (target != nullptr) {
			...
			if (creature != nullptr) {
				...
				if (targetGhost != nullptr) {
					...
					if (rank >= 0 and rank <= 11) {
						rankString = " (Rank " + String::valueOf(rank) + ")";
					}
					else if (rank == -1 && creature->hasSkill("force_title_jedi_rank_02"))
						rankString = " (Padawan)";
					else
						rankString = " (non-Jedi)";
				}
			...
			mission->setMissionTitle("Player Bounty", " Eliminate the player target " + name + rankString);
...
bool MissionManagerImplementation::isBountyValidForPlayer(CreatureObject* player, PlayerBounty* bounty) {
	...
	/*
	if (!creature->checkCooldownRecovery("killedBH")) {	//10 minute cooldown after killing BH
		return false;
	}
	*/
	...
	if (zone != nullptr) {
		...
		if (zone->getZoneName() == "corellia") {   //Check to see if target is unattackable in Swamptown neutral zone
