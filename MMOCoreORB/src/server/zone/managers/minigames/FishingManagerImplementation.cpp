	...
	if (fish > 93){ // NON-FISH ITEM WAS CAUGHT!
		...
		if (badLuck == 0) {
			...
			if (lootFishObject != nullptr) {
				...
				length = round(System::random(length)+quality);

				//player->sendSystemMessage("Fish length = " + String::valueOf(length));

				lootFishObject->setAttributes(name, zone->getZoneName(), time, length / 100.0f);
				...
				int amount = 25 + System::random(25) + 6.25 * factor;  //25 minimum + 25 random + (skill factor * 6.25 = 50 for ranger tracking 4)

				float lengthBonus = 1.0f;
				switch ((int)length) {
					case 300 ... 400:
						lengthBonus = 1 + (length / 100.0 / 3.25);
						break;
					case 200 ... 299:
						lengthBonus = 1 + (length / 100.0 / 4.0);
						break;
					case 100 ... 199:
						lengthBonus = 1 + (length / 100.0 / 5.0);
						break;
					case 0 ... 99:
						lengthBonus = Math::max(0.80f, length / 100.0f);
						break;
					default:
						lengthBonus = 1.0f;
						break;
