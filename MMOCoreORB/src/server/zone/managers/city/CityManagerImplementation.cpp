...
void CityManagerImplementation::processCityUpdate(CityRegion* city) {
	...
	try {
		...
		city->removeAmenitiesOutsideCity(radius);  //Cleanup anything moved outside the city limits
...
void CityManagerImplementation::sendCityStatusReport(CreatureObject* creature, const String& planetName) {
	...
	report << "==============================================" << endl;
	report << "City, Rank, Specialization, Location" << endl;
	report << "-----------------------------------------------------------------------------------" << endl;
	...
	for (int i = 0; i < cities.size(); ++i) {
		...
		report << ", x: " << String::valueOf(ceil(city->getPositionX())) << " y:" << String::valueOf(ceil(city->getPositionY())) << endl;
