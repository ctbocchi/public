...
int CreatureManagerImplementation::notifyDestruction(TangibleObject* destructor, AiAgent* destructedObject, int condition, bool isCombatAction) {
	...
	try {
		...
		//Core::getTaskManager()->executeTask([strongDestructed] () {
	...
	if (destructor->isPet()) {
		...
		String petName = "";
		String ownerName = "";
		String destructedName = "";

		ManagedReference<CreatureObject*> attackerCreature = cast<CreatureObject*>(destructor);

		if (attackerCreature != nullptr) {

			petName = attackerCreature->getCreatureName().toString();

			ManagedReference<CreatureObject*> owner = attackerCreature->getLinkedCreature().get();

			if (owner != nullptr && owner->isPlayerCreature())
				ownerName = owner->getCreatureName().toString();
		}

		if (destructedObject != nullptr) {

			if (destructedObject->isCreature())
				destructedName = destructedObject->getObjectName()->getStringID();
			else
				destructedName = destructedObject->getCreatureName().toString();
		}

		
		error("notifyDestruction - Pet = " + petName + ", petOID = " + String::valueOf(attackerCreature->getObjectID()) + 
					", owner = " + ownerName + ", destructed object = " + destructedName + ", destructedOID = " + String::valueOf(destructedObject->getObjectID()));
...
void CreatureManagerImplementation::harvest(Creature* creature, CreatureObject* player, int selectedID) {
	...
	float adjustedRange = 7.0f + Math::min(creature->getTemplateRadius() / 1.5f, 17.0f);
