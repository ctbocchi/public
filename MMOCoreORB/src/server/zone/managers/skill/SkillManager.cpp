...
void SkillManager::removeSkillRelatedMissions(CreatureObject* creature, Skill* skill) {
	if(skill->getSkillName().hashCode() == STRING_HASHCODE("combat_bountyhunter_investigation_03") || skill->getSkillName().hashCode() == STRING_HASHCODE("mandalorian_master")) {
...
bool SkillManager::surrenderSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool ignoreRequirements, bool setRank) {
	...
	if (ghost != nullptr) {
		...
		if (skillName.beginsWith("force_")) {
			...
			//Reset FRS to zero if dropping Jedi Knight or either Rank Novice Box
			if (skillName == "force_rank_dark_novice" || skillName == "force_rank_light_novice" || skillName == "force_title_jedi_rank_03") {
				int frsXP = ghost->getExperience("force_rank_xp");
				if (frsXP >  0) {
					ManagedReference<PlayerManager*> playerManager = creature->getZoneServer()->getPlayerManager();
					if (playerManager != nullptr)
						playerManager->awardExperience(creature, "force_rank_xp", frsXP * -1);
				}
			}
		}
