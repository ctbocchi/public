...
bool ZoneContainerComponent::insertActiveArea(Zone* newZone, ActiveArea* activeArea) const {
	...
	//Locker clocker(activeArea, newZone);
...
bool ZoneContainerComponent::transferObject(SceneObject* sceneObject, SceneObject* object, int containmentType, bool notifyClient, bool allowOverflow, bool notifyRoot) const {
	...
	if (newZone == nullptr || object == nullptr)
		return false;
	...
	//Locker clocker(object, newZone);
	newZone->addSceneObject(object);
	//clocker.release();
	...
	newZone->insert(object);

	newZone->inRange(object, ZoneServer::CLOSEOBJECTRANGE);

	TangibleObject* tanoObject = object->asTangibleObject();
	if (tanoObject != nullptr) {
		newZone->updateActiveAreas(tanoObject);
		...
		newZone->getInRangeNavMeshes(worldPos.getX(), worldPos.getY(), &meshes, false);
		...
		if (theater != nullptr && theater->shouldFlattenTheater()) {
			newZone->getPlanetManager()->getTerrainManager()->addTerrainModification(object->getWorldPositionX(), object->getWorldPositionY(), "terrain/poi_small.lay", object->getObjectID());
		}
	}
	...
	object->notifyInsertToZone(newZone);
