...
Reference<TangibleObject*> FactoryCrateImplementation::getPrototype() {
	...
	if (getContainerObjectsSize() == 0) {
		...
		error("FactoryCrateImplementation::getPrototype - Crate name = " + getCustomObjectName().toString() + ", objectID = " + String::valueOf(getObjectID()));

		ManagedReference<CreatureObject*> playerCreature = getParentRecursively(SceneObjectType::PLAYERCREATURE).castTo<CreatureObject*>();
		if (playerCreature != nullptr) {
			error("FactoryCrateImplementation::getPrototype - Player name = " + playerCreature->getFirstName());
		}
		else {
			Zone* zone = getZone();
			if (zone != nullptr) {
				String zoneName = zone->getZoneName();
				error("FactoryCrateImplementation::getPrototype - Zone = " + zoneName + ", waypoint = " + String::valueOf(getWorldPositionX()) + "," + String::valueOf(getWorldPositionY()));
			}
		}
	...
	if(prototype == nullptr) {
		error("FactoryCrateImplementation::getPrototype - prototype is nullptr");
		error("Crate name = " + getCustomObjectName().toString());
		return nullptr;
	}

	if(!prototype->isTangibleObject()) {
		error("FactoryCrateImplementation::getPrototype - prototype is non-tangible item");
		error("Crate name = " + getCustomObjectName().toString());
		return nullptr;
	}
...
bool FactoryCrateImplementation::extractObjectToInventory(CreatureObject* player) {
	...
	if (prototype == nullptr) {
		error("FactoryCrateImplementation::extractObjectToInventory - prototype is nullptr");
		return false;
	}

	if (!prototype->isTangibleObject()) {
		error("FactoryCrateImplementation::extractObjectToInventory - prototype is non-tangible item");
		return false;
	}

	if (inventory == nullptr) {
		error("FactoryCrateImplementation::extractObjectToInventory - inventory is nullptr");
		return false;
	}
...
Reference<TangibleObject*> FactoryCrateImplementation::extractObject(int count) {
	...
	if(prototype == nullptr) {
		error("FactoryCrateImplementation::extractObject - prototype is nullptr");
		return nullptr;
	}

	if(!prototype->isTangibleObject()) {
		error("FactoryCrateImplementation::extractObject - prototype is non-tangible item");
		return nullptr;
	}
