	...
	if (object->isTangibleObject() && containmentType == 4) {
		...
		if (object->isWeaponObject()) {
			...
			if (weapon->isOneHandMeleeWeapon()) {
				if (weapon->getObjectName()->getStringID() == "sword_02") {
					if (weapon->getArmorPiercing() != 1) {
						Locker locker(weapon);
						weapon->setArmorPiercing(1);
						creo->sendSystemMessage("Your 1-Handed Curved Sword has been upgraded to Light armor piercing.");
						locker.release();
					}
				}
			}
