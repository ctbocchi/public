...
public:
	...
	int doJediSelfBuffCommand(CreatureObject* creature) const {
		...
		if (creature->hasBuff(buffCRC)) {
			...
			if (buffCRC == BuffCRC::BuffCRC::JEDI_FORCE_ARMOR_1 || buffCRC == BuffCRC::BuffCRC::JEDI_FORCE_ARMOR_2 || 	//these are the buffs we want to toggle off rather than refresh
