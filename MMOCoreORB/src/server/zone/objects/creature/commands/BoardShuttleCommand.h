...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (region == nullptr || region->isClientRegion()) {
			...
			// the proper location for arrival is along a 30 degree arc centered on the shuttle's facing axis, between 14.5 and 17.5 meters out
			dirDegrees = dirDegrees - 15 + System::random(30);
			float dirRadians = dirDegrees * M_PI / 180;
			float distance = 14.5 + System::random(3);
