...
int ForceHealQueueCommand::runCommand(CreatureObject* creature, CreatureObject* targetCreature) const {
	...
	if (creature->hasSkill("force_discipline_healing_master") && (creature->hasSkill("force_rank_light_novice") || creature->hasSkill("force_rank_dark_novice")) && HAMhealPerformed) {
		...
		for (int i = 0; i < closeObjects.size(); i++) {
			...
			if (chainHealed) { //If creature was healed, increase the iteration count
				iterations++;
				CombatManager::instance()->broadcastCombatSpam(creature, chainCreatureTarget, nullptr, 0, "jedi_spam", "chainheal", 0);
	...
	if (selfHeal && creature->hasSkill("force_discipline_healing_master") && healAmount == -1) { //Master healer performing totalHealSelf removes force debuffs
	...
	if (healPerformed) {
		...
		if (!selfHeal && healAmount == -1)
			CombatManager::instance()->broadcastCombatSpam(creature, targetCreature, nullptr, 0, "jedi_spam", "totalhealother", 0);
