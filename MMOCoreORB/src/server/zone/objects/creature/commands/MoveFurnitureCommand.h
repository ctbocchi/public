	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!(ghost->getAdminLevel() == 15))  { //// Restrictions for non-admin characters
			...
			if (buildingObject == nullptr) {  // We're outside
				...
				if (!isDeco && !obj->isMissionTerminal() && !obj->getObjectTemplate()->getFullTemplateString().contains("trainer")) {
		...
		if (isDeco && city != nullptr) {  // Non-admin, moving deco in a city
			Vector3 cityCenter(city->getPositionX(), city->getPositionY(), 0);
			if (endPoint.distanceTo(cityCenter) > city->getRadius()) {
				creature->sendSystemMessage("You cannot move city decorations outside of the city limit radius."); 
				return GENERALERROR;
			}
		}
