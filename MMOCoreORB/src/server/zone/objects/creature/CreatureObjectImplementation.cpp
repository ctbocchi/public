...
*/
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* attackerCreature, bool bypassDeadCheck) {
	if (attackerCreature == nullptr || attackerCreature == asCreatureObject())
		return false;

	if ((!bypassDeadCheck && isDead()) || isInvisible())
		return false;

	if (attackerCreature->getZoneUnsafe() != getZoneUnsafe())
		return false;
	...
	if (attackerCreature->isAiAgent()) {
		if (attackerCreature->isPet()) {
			ManagedReference<CreatureObject*> owner = attackerCreature->getLinkedCreature().get();
				if (owner == nullptr) {
					return false;
				}
			ManagedReference<PetControlDevice*> pcd = attackerCreature->getControlDevice().get().castTo<PetControlDevice*>();
		...
		if(!attackerCreature->isRebel() && !attackerCreature->isImperial())
			return true;

		if(getFaction() == 0 || getFaction() == attackerCreature->getFaction())
			return false;
	...
	if (ghost == nullptr || attackerGhost == nullptr)
		return false;

	bool areInDuel = (ghost->requestedDuelTo(attackerCreature) && attackerGhost->requestedDuelTo(asCreatureObject()));

	if (areInDuel)
		return true;

	if (attackerCreature->hasBountyMissionFor(asCreatureObject())){
			return true;
	}

	if (ghost->hasBhTef() && hasBountyMissionFor(attackerCreature)) {
		return true;
		/*
		Locker locker(attackerCreature);	//Possibly fix for multiple BH issue with TEF
		ThreatMap* threatMap = attackerCreature->getThreatMap();
		locker.release();
		if (threatMap != nullptr) {
			if (threatMap->find(asCreatureObject()) != -1) {
				return true;
			}
		}
		*/
	...
	if (getGroupID() != 0 && getGroupID() == attackerCreature->getGroupID())
		return false;

	if ((pvpStatusBitmask & CreatureFlag::OVERT) && (attackerCreature->getPvpStatusBitmask() & CreatureFlag::OVERT) && attackerCreature->getFaction() != getFaction())
		return true;

	ManagedReference<GuildObject*> guildObject = guild.get();
	if (guildObject != nullptr && guildObject->isInWaringGuild(attackerCreature))
		return true;
