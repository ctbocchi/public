...
int AiAgentImplementation::calculateAttackMinDamage(int level) {
	...
	if (isDroidObject()) {
		...
		//error("Min Damage = " + String::valueOf(calculateAttackMaxDamage(level) - 10));
		minDmg = calculateAttackMaxDamage(level) - 10;
		//error("MinDmg = " + String::valueOf(minDmg));
	}
	else {
...
int AiAgentImplementation::calculateAttackMaxDamage(int level) {
	...
	if (isDroidObject()) {
		...
		/* error("Template Max Damage = " + String::valueOf(templateMaxDamage));
		error("Level = " + String::valueOf(level));
		error("Damage based on level (50 + level*14) = " + String::valueOf(50 + level*14)); */
		dmg = Math::max((float)templateMaxDamage, (50.0f + level * 14));
		// error("Max Damage = " + String::valueOf(dmg));
	}
	else {
...
void AiAgentImplementation::setLevel(int lvl, bool randomHam) {
	...
	//error("baseLevel = " + String::valueOf(baseLevel) + ", level = " + String::valueOf(level));

	float ratio = ((float)level) / (float)baseLevel;

	// error("ratio = " + String::valueOf(ratio));

	if (baseLevel == lvl)
		return;

	float maxDmg = calculateAttackMaxDamage(baseLevel);
	float minDmg = Math::min((float)maxDmg - 10.0f, (float)calculateAttackMinDamage(baseLevel));
	...
	int baseHamMax = Math::min(((float)getHamMaximum()), 36000.0f) * ratio;
	int baseHam = Math::min(((float)getHamBase()), 34000.0f) * ratio;

	if (npcTemplate->getTemplateName().contains("varactyl")) {  //15% Bonus HAM for varactyls
		baseHamMax *= 1.15;
		baseHam *= 1.15;
	}
	int petHam = System::random(baseHamMax - baseHam) + baseHam;
	...
	for (int i = 0; i < 9; ++i) {
		if (i % 3 == 0) {
			if (randomHam)
				...
				ham = petHam;  //   ((baseHamMax - baseHam) / 2) + baseHam;
...
bool AiAgentImplementation::validateTarget(SceneObject* target) {
	...
	/*if (targetCreatureObject != nullptr && targetCreatureObject->getSkillMod("avoid_incapacitation") > 0 && (!homeLocation.isInRange(asAiAgent(), 32) || !target->isInRange(asAiAgent(), 32))) //Mobs are not interested in chasing invincible targets.
		return false; */
...
bool AiAgentImplementation::isScentMasked(CreatureObject* target) {
	...
	success = System::random(100) <= mod - (float)creatureLevel / ((float)camoSkill / 100.0f) / 56.0f;
...
bool AiAgentImplementation::isConcealed(CreatureObject* target) {
	...
	if (!isCreature())
		creatureLevel *= 1.5;  // NPC's are smarter than critters

	success = System::random(100) <= mod - ((float)creatureLevel / 2.0f) / ((float)camoSkill / 100.0f) / 43.0f;
...
float AiAgentImplementation::getEffectiveResist() {
	float maxResist = Math::max(getAcid(),getBlast());
	maxResist = Math::max(maxResist, getCold());
	maxResist = Math::max(maxResist, getElectricity());
	maxResist = Math::max(maxResist, getEnergy());
	maxResist = Math::max(maxResist, getHeat());
	maxResist = Math::max(maxResist, getKinetic());
	maxResist = Math::max(maxResist, getLightSaber());
	maxResist = Math::max(maxResist, getStun());
	return (maxResist > 0) ? maxResist : 0;
