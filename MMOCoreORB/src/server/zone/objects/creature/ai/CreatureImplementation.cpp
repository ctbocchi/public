...
bool CreatureImplementation::canHarvestMe(CreatureObject* player) {
	...
	if (!player->isInRange(_this.getReferenceUnsafeStaticCast(), adjustedRange))
		return false;

	if(player->isInCombat() || !player->hasSkill("outdoors_scout_novice") || player->isDead() || player->isIncapacitated() || isPet())
		return false;
...
void CreatureImplementation::setPetLevel(int newLevel) {
	...
	//error("Min-Max damage from template  = " + String::valueOf(getMinDamage) + "-" String::valueOf(getMaxDamage));
	...
	//error("newLevel = " + String::valueOf(newLevel) +  " baseLevel = " + String::valueOf(baseLevel) +  " ratio = " + String::valueOf(ratio));
