...
uint32 DamageOverTime::doForceChokeTick(CreatureObject* victim, CreatureObject* attacker) {
	...
	if (armor != nullptr && !armor->isVulnerable(SharedWeaponObjectTemplate::LIGHTSABER)) {
		...
		if (attacker->isPlayerCreature())	//Force Choke now has inherent AP2 for players only
			armorPiercing = 2;

		if (attacker->isPlayerCreature() && victim->hasBuff(BuffCRC::JEDI_FORCEBREACH))		// Increase DOT damage by one AP level when breached
			armorPiercing++;
