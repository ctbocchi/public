...
function MellichaeOutroTheater:onPowerShrineHit(pSceneObject, pKiller, damage)
	if (pSceneObject == nil or pKiller == nil) then
		return 0
	end

	local ownerID = readData(SceneObject(pSceneObject):getObjectID() .. "questOwner")
	local pPlayer = util:petOwner(pKiller)
	local killerID = SceneObject(pPlayer):getObjectID()
	if ownerID ~= nil and killerID ~= nil then
		util:cw("ownerID=" .. ownerID, 1)
		util:cw("killerID=" .. killerID, 1)
		if ownerID ~= killerID then
		...
		util:cw("owner or killer is nil", 1)
	end
