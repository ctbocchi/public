...
function VillageCommunityCrafting:doEndOfPhasePrizes()
	...
	for i = 1, #ingredientList, 1 do
		...
		if #statTable ~= 0 then
			...
			if (pQualityWinner ~= nil) then
				...
				util:l("Village Crafting phase " .. currentPhase .. " qualityWinner", pQualityWinner, 3)
				if pInventory ~= nil then
					local pItem = giveItem(pInventory, prizeTable.quality, true)
					if pItem ~= nil and pItem then
						util:l("Village Crafting phase " .. currentPhase .. " qualityWinner: Crafting tool delivered", pQualityWinner, 3)
					else
						util:l("Village Crafting phase " .. currentPhase .. " qualityWinner: Crafting tool FAILURE", pQualityWinner, 3)
					end
				end
			end
			...
			if (pQuantityWinner ~= nil) then
				...
				util:l("Village Crafting phase " .. currentPhase .. " quantityWinner", pQuantityWinner, 2)
				if pInventory ~= nil then					
					local pItem = giveItem(pInventory, prizeTable.quantity, true)
					if pItem ~= nil and pItem then
						util:l("Village Crafting phase " .. currentPhase .. " quantityWinner: Crafting tool delivered", pQualityWinner, 3)
					else
						util:l("Village Crafting phase " .. currentPhase .. " quantityWinner: Crafting tool FAILURE", pQualityWinner, 3)
					end
				end
			end
		end
	...
	if (#overallQuality > 0) then
		...
		if (pQualityWinner ~= nil) then
			...
			util:l("Village Crafting phase " .. currentPhase .. " qualityWinner", pQualityWinner, 3)
			if pInventory ~= nil then
				local pItem = giveItem(pInventory, self.phaseInfo[currentPhase].overallPrize.quality, true)
				if pItem ~= nil and pItem then
					util:l("Village Crafting phase " .. currentPhase .. " qualityWinner: Crafting tool delivered", pQualityWinner, 3)
				else
					util:l("Village Crafting phase " .. currentPhase .. " qualityWinner: Crafting tool FAILURE", pQualityWinner, 3)
	...
	if (#overallQuantity > 0) then
		...
		if (pQuantityWinner ~= nil) then
			local pInventory = CreatureObject(pQuantityWinner):getSlottedObject("inventory")
			util:l("Village Crafting phase " .. currentPhase .. " quantityWinner", pQualityWinner, 2)
			if pInventory ~= nil then
				local pItem = giveItem(pInventory, self.phaseInfo[currentPhase].overallPrize.quantity, true)
				if pItem ~= nil and pItem then
					util:l("Village Crafting phase " .. currentPhase .. " quantityWinner: Crafting tool delivered", pQualityWinner, 3)
				else
					util:l("Village Crafting phase " .. currentPhase .. " quantityWinner: Crafting tool FAILURE", pQualityWinner, 3)
...
function QtQcContainerComponent:transferObject(pContainer, pObj, slot)
	...
	if (ingredientsNeeded == 0) then
		...
		if (currentPhase == 2 and not QuestManager.hasCompletedQuest(pPlayer, QuestManager.quests.FS_PHASE_2_CRAFT_DEFENSES_MAIN)) then
			...
			util:l("Phase 2 crafting complete",pPlayer, 2)
		...
		elseif (currentPhase == 3 and not QuestManager.hasCompletedQuest(pPlayer, QuestManager.quests.FS_PHASE_3_CRAFT_SHIELDS_MAIN)) then
			...
			util:l("Phase 3 crafting complete",pPlayer, 2)
